...
def load_volfile(
    ...
    #load mat file
        ...
        mat_file = mat73.loadmat(filename)
        for dic in mat_file.keys():
            vol = mat_file[dic]
        affine = None
    else:
...
def pad(array, shape):
    ...
    # ##temporary normalize
    # padded_norm = (padded - np.min(padded))/(np.max(padded) - np.min(padded))
    
    # return np.nan_to_num(padded_norm), slices
    # return array , slices
